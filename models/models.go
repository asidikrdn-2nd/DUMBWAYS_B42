...
func GetAllDataProject() []DataProject {
	...
	dataQuery, errQuery := db.Query(context.Background(), "SELECT id, name, start_date, end_date, description, technologies, image FROM tb_projects ORDER BY id ASC")
	if errQuery != nil {
...
func AddProject(newDataProject DataProject) {
	...
	_, err = db.Exec(context.Background(),
		...
		newDataProject.ProjectName, newDataProject.StartDate, newDataProject.EndDate, newDataProject.ProjectDesc, newDataProject.Tech, newDataProject.Img)
	if err != nil {
		panic(err.Error())
	}
...
func DeleteProject(idProject int) {
	...
	_, err = db.Exec(context.Background(), "DELETE FROM tb_projects WHERE id=$1", idProject)
	if err != nil {
		panic(err.Error())
	}
...
func GetUserDataByEmail(email string) User {
	...
	// mengambil 1 baris data dari database
	errScanRow := db.QueryRow(context.Background(), "SELECT id, name, email, password FROM tb_users WHERE email = $1", email).Scan(&UserAccount.Id, &UserAccount.Name, &UserAccount.Email, &UserAccount.Password)
	if errScanRow != nil {
		panic(errScanRow.Error())
	}
	/**
